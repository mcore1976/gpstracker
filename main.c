...
// for 8Meg : -U lfuse:w:0xe2:m     on ATMEGA328P
...
void sleepnow(void)
{
    ...
    // PD2 (PCINT0 pin) is now an input pin

    PORTD |= (1 << PORTD2); // turn ON the Pull-up 
    // PD2 / INT0 is now an input with pull-up enabled
                ...
                if (readline()>0)
                   {
                    ...
                    if (is_in_rx_buffer(response, buf) == 1) 
                     { initialized = 1; 
                      ...
                      readcellid();
                      // delay 25 sec for terminal to fix on the network again
                      delay_sec(25);
                      // check registration status and provision APN settings
                      initialized = checkregistration();
                      initialized = provisiongprs();
                      initialized = 1;
                      delay_sec(2);
