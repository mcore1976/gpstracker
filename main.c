 ...
 * d) this version also scans neigboring 2G cells and sends 3 cells information
 *
...
const char CELLS[] PROGMEM = {"NEAREST CELLS INFO - \n"};
...
const char SCANCELLS[] PROGMEM = {"AT+CNETSCAN\n\r"};
...
// ----------------------------------------------------------------------------------------
// read 3 nearest CELLIDs with RXlev for triangulation, put all of it to buffer for SMS 
// ----------------------------------------------------------------------------------------
uint8_t readcellid()
{
  uint16_t char1;
  uint8_t i;
  // put to beginning of buffer
  cellid_response_pos = 0;

 // analyze first 3 lines of output
 for(i=0; i<3; i++)
   {
  // wait for first COMMA

      // wait for first COMMA sign - bypass network name
      do { 
           char1 = receive_uart();
         } while ( char1 != ',' );
      // if COMMA detected - copy- MCC 3 digits info
      do  { 
           char1 = receive_uart();
           cellid_response[cellid_response_pos] = char1; 
           cellid_response_pos++;
         } while ( char1 != ',' );
      // if COMMA detected - copy - MNC 3 digits info
      do  { 
           char1 = receive_uart();
           cellid_response[cellid_response_pos] = char1; 
           cellid_response_pos++;
      ...
      // if COMMA DETECTED start to copy  RxLEV value 
      do  { 
           char1 = receive_uart();
           cellid_response[cellid_response_pos] = char1; 
           cellid_response_pos++;
         } while ( char1 != ',' );
      // COPY CELL ID AFTER NEXT COMMA
      do  { 
           char1 = receive_uart();
           cellid_response[cellid_response_pos] = char1; 
           cellid_response_pos++;
  ...
  // Putt  New Line  at the end of this line and wait for next line of Cell ID NetScan output
           cellid_response[cellid_response_pos-1] = 0x0a; 
  // end of FOR loop
  };
     // put NULL to end of all cellid info string
   cellid_response[cellid_response_pos-1] = NULL; 
   cellid_response_pos=0;
return (1);
}
  ...
  uint8_t initialized2, attempt2, nbrminutes;
      ...
      nbrminutes = 0;
		...
		delay_sec(180);
                ...
                // end of DO loop
                } while ( (initialized2 == 0) && (attempt2 < 48) );   // stop after 48 hours of searching
                ...
                if (readline()>0)
                   {
                    ...
                    { initialized = 1; 
                      ...
                      uart_puts_P(HANGUP);
                      delay_sec(1);
                      // perform NETSCAN to note all neighboring 2G cells and note them
                      // the SIM800L will disconnect from 2G while scanning cells
                      uart_puts_P(SCANCELLS);
                      // wait for cellid list 
                      readcellid();
                      // delay 120 sec for terminal to get to 2G network again
                      delay_sec(120);
                      uart_puts_P(SHOW_REGISTRATION);
                      delay_sec(2);
                      ...
                      provisiongprs();
                      delay_sec(2);
                      // send info about cells over SMS and then proceed further with Google API
	              ...
	              uart_puts(phonenumber);  // send phone number received from CLIP
                      uart_puts_P(CRLF);   			   
                      delay_sec(1); 
                      // put cell IDs for triangualation 
                      uart_puts_P(CELLS);
                      uart_puts(cellid_response);  // from buffer
                      delay_sec(1); 
                      // end the SMS message
                      send_uart(26);   // ctrl Z to end SMS
                      // turn ON the flag for GPRS based location checking
                      initialized = 1;
                      } // end of IF
