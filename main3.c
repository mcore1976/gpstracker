/* ---------------------------------------------------------------------------
 * AGPS car tracker on ATTINY2313 / ATTINY2313A + SIM800L module version 2.0
 ...
 * must be connected to RIN ping on SIM800L module
 * RING low voltage state initiates interrupt INT0 and wakes up ATTINY 
 * this release sends also Battery status - voltage in mV
 * ---------------------------------------------------------------------------
 */
...
//const char SMS2[] PROGMEM = {"AT+CMGS=\"+48"};   // no ONLY FOR ORANGE PL because it does not show +48 in CLIP identification
...
// when interrupt from INT0 disable next interrupts from RING pin of SIM800L
ISR(INT0_vect)
{
  ...
  // check status of all functions
     ...
     // neverending LOOP
       ...
       while (1) {
        ...
        delay_sec(10);
        // enter sleepmode on SIM800L and ATTINY2313 again for power saving
        uart_puts_P(SLEEPON);
        delay_sec(1);
        sleepnow(); 
